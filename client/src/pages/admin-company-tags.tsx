...
export default function AdminCompanyTags() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: InsertCompanyTag) => {
      const res = await apiRequest("POST", "/api/admin/company-tags", data);
      return res.json();
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<InsertCompanyTag> }) => {
      const res = await apiRequest("PATCH", `/api/admin/company-tags/${id}`, data);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/company-tags"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await apiRequest("DELETE", `/api/admin/company-tags/${id}`);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/company-tags"] });
